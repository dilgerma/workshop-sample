...
export const subscribeStream = async (stream: string, subscription: Subscription) => {
    subscriptions.set(stream, [...subscriptions.get(stream) ?? [], subscription])
    let result = await findEventStore().readStream(stream)
    subscription(result?.currentStreamVersion ?? BigInt(0), result?.events ?? []);
}
